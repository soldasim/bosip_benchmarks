...
function queue_score_calc(problem::AbstractProblem, run_name::String, metric::Type{<:DistributionMetric};
    ...
    for iter_file in iter_files
        ...
        @info "Queuing calculation of \"$(metric)\" for problem:\"$(problem_name)\", run_name:\"$(run_name)\", run_idx:\"$(run_idx)\""
        # TODO --mem (the code was failing with the `SimpleProblem` with the default 4G memory)
        job_name = "$(metric)_$(problem_name)_$(run_name)_$(run_idx)"
        Base.run(`sbatch -p cpulong --mem=16G --job-name=$job_name cluster_scripts/run_score.sh $problem_name $run_name $run_idx $metric`)
    end
