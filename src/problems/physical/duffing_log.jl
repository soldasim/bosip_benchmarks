...
"""
struct LogDuffingProblem <: AbstractProblem end

module LogDuffingModule
...
simulator(::LogDuffingProblem) = model_target

domain(::LogDuffingProblem) = Domain(;
    bounds = _get_bounds(),
)

# TODO loglike
# likelihood(::LogDuffingProblem) = NormalLikelihood(; z_obs, std_obs)
likelihood(::LogDuffingProblem) = ExpLikelihood()

# TODO loglike
# prior_mean(::LogDuffingProblem) = _get_prior_mean()
prior_mean(::LogDuffingProblem) = [0.]

x_prior(::LogDuffingProblem) = _get_trunc_x_prior()

# TODO loglike
# est_amplitude(::LogDuffingProblem) = _get_est_amplitude()
est_amplitude(::LogDuffingProblem) = [1000.] # TODO ???
...
function model_target(x_)    
    sol = duffing_simulation(x_)
    positions, velocities, times = extract_measurements(sol)
    loglike = logpdf(MvNormal(positions, std_obs), z_obs)
    return [loglike]
end
